...
int ACQPS[16]   = {13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13};
...
// Initialize ADC for DMC Kit Rev 1.1 	 
	ChSel[0]=1;		// Dummy meas. avoid 1st sample issue Rev0 Piccolo*/
...
// ------------------------------------------------------------------------------
	...
	// Due to the PGA operating range [0.65, 2.65], all ADC conversion results
	// must be scaled to [0.0, 3.3].  This is done by multiplying by 1.65.
	clarke1.As = -_IQmpy2(_IQmpy(_IQ12toIQ(AdcResult.ADCRESULT1)-offsetA, _IQ(1.65))); // Phase A curr.
	clarke1.Bs = -_IQmpy2(_IQmpy(_IQ12toIQ(AdcResult.ADCRESULT2)-offsetA, _IQ(1.65))); // Phase B curr.
...
//    Connect inputs of the DATALOG module 
// ------------------------------------------------------------------------------
    DlogCh1 = (int16)_IQtoIQ15(fe1.PsiQrS);
    DlogCh2 = (int16)_IQtoIQ15(fe1.PsiDrS);
...
//===========================================================================
	...
	// Set the CTRIPM1 as source for trip signals.
	// Code uses DCAH to control EPWM channel A &
	// DCBH to control EPWM channel B.
	EPwm1Regs.DCTRIPSEL.bit.DCAHCOMPSEL = 0x8;
	EPwm1Regs.DCTRIPSEL.bit.DCBHCOMPSEL = 0x8;
	EPwm2Regs.DCTRIPSEL.bit.DCAHCOMPSEL = 0x8;
	EPwm2Regs.DCTRIPSEL.bit.DCBHCOMPSEL = 0x8;
	EPwm3Regs.DCTRIPSEL.bit.DCAHCOMPSEL = 0x8;
	EPwm3Regs.DCTRIPSEL.bit.DCBHCOMPSEL = 0x8;

	// Make the DCAH/L signals trigger an Event
	EPwm1Regs.TZDCSEL.bit.DCAEVT1 = 0x2;// Trigger event if DCAH is high
	EPwm1Regs.TZDCSEL.bit.DCBEVT1 = 0x2;// Trigger event if DCBH is high
	EPwm2Regs.TZDCSEL.bit.DCAEVT1 = 0x2;// Trigger event if DCAH is high
	EPwm2Regs.TZDCSEL.bit.DCBEVT1 = 0x2;// Trigger event if DCBH is high
	EPwm3Regs.TZDCSEL.bit.DCAEVT1 = 0x2;// Trigger event if DCAH is high
	EPwm3Regs.TZDCSEL.bit.DCBEVT1 = 0x2;// Trigger event if DCBH is high

	// Bypass event filtering & CLK synchronization
	...
	EPwm2Regs.DCBCTL.bit.EVT1FRCSYNCSEL = 1;
	EPwm3Regs.DCACTL.bit.EVT1SRCSEL = 0;
	EPwm3Regs.DCACTL.bit.EVT1FRCSYNCSEL = 1;
	EPwm3Regs.DCBCTL.bit.EVT1SRCSEL = 0;
	EPwm3Regs.DCBCTL.bit.EVT1FRCSYNCSEL = 1;

	// Disable ePWM if event is triggered.
	EPwm1Regs.TZCTL.bit.DCAEVT1 = 0x2;		// Force EPWMxA low
	EPwm1Regs.TZCTL.bit.DCBEVT1 = 0x2;		// Force EPWMxB low
	EPwm2Regs.TZCTL.bit.DCAEVT1 = 0x2;		// Force EPWMxA low
	EPwm2Regs.TZCTL.bit.DCBEVT1 = 0x2;		// Force EPWMxB low
	EPwm3Regs.TZCTL.bit.DCAEVT1 = 0x2;		// Force EPWMxA low
	EPwm3Regs.TZCTL.bit.DCBEVT1 = 0x2;		// Force EPWMxB low
	...
	EPwm2Regs.TZCLR.all = 0x00FF;
	EPwm3Regs.TZCLR.all = 0x00FF;
