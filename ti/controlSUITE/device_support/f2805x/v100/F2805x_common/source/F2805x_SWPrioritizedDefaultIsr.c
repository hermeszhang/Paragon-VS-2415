...
//
// TITLE:   F2805x Device Default Software Prioritized Interrupt Service Routines.
//
//###########################################################################
//
...
#if (G37PL != 0)
__interrupt void EPWM7_INT_ISR(void)     // ePWM4 Interrupt
{
	// Set __interrupt priority:
	volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER3.all;
	IER |= M_INT3;
...
/* Uncomment the below if ADCINT1 and ADCINT2 in the 10.1 and 10.2
   PIE __interrupt spaces are enabled. Then comment out the equivalent
   1.1 and 1.2 __interrupt service routines. */
...
#if (G121PL != 0)
__interrupt void XINT3_ISR(void)     // External __interrupt
{
	// Set __interrupt priority:
