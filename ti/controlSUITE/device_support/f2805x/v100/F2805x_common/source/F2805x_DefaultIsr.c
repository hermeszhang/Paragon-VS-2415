...
// with the user's own ISR routines for each __interrupt.  Since these shell ISRs
...
__interrupt void ADCINT1_ISR(void)   // ADC  (Can also be ISR for INT10.1 when enabled)
{
  ...
  // To receive more __interrupts from this PIE group, acknowledge this __interrupt
